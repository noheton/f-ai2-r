...
% 4.7 Live preview as oversight affordance — the skill's "show paper"
%     step republishes the current typeset build to a stable URL after
%     every editing round (Figure~\ref{fig:environment}, right panel).
%     Effective human oversight of a fast-moving AI collaborator
%     requires a continuously current view of the deliverable, not a
%     repository dig; the EU AI Act's human-oversight expectation
%     becomes a user-interface property of the workflow.
% 4.8 Structure before prose — the authoring discipline observed here:
%     the agent first lays down each section's principal structure
%     (argument flow, evidence, figure and citation plan) as reviewable
%     skeleton, and prose is a separate later pass. The operator can
%     contest structure cheaply (as happened with the ladder semantics)
%     before any prose exists to defend; both passes are logged, so
%     the audit trail shows WHICH structural decisions preceded which
%     text.
