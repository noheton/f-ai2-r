...
\emergencystretch=2em % single-column lines + unbreakable \texttt terms
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{tikz}
...
\usepackage{xcolor}
\usepackage{url}
% QSS first-submission format: author-year citations (numbered
% references are explicitly disallowed) and line numbers (suggested).
\usepackage[round]{natbib}
\renewcommand{\cite}{\citep} % section prose uses \cite as a bracketed reference
\usepackage{lineno}
...
\graphicspath{{../paper/}}
\input{../paper/metrics}
% Render the extended (journal) material in the shared sections.
\newif\ifextended
\extendedtrue
...
agent skills; FAIR; human oversight
\end{abstract}

\input{../paper/sections/01-introduction}
\input{../paper/sections/02-background}
\input{../paper/sections/03-dilution}
\input{../paper/sections/04-method}
\input{../paper/sections/05-skill}
\input{../paper/sections/06-meta-experiment}
\input{../paper/sections/07-discussion}
\input{../paper/sections/08-conclusion}

\section*{Acknowledgment and AI Transparency Statement}
The ideas in this paper were sharpened through earlier
experiments~\cite{obscurity-is-dead, fair2r-repo}; through
Christian Schr\"oer's Augsburg seminar on responsibility (winter
term 2007/08), where H\"offe's four-place relation entered the
author's thinking eighteen years before this
record~\cite{verantwortung2008}; through discussions
...
\begingroup\itshape
\input{../paper/disclosure}
\endgroup

\emph{Author contributions (CRediT):} Florian Krebs:
conceptualization, methodology, investigation, validation, data
curation, writing (original draft), writing (review and editing),
supervision. All roles were exercised with AI assistance operating
under the author's direction, as disclosed in the transparency
statement above.
...
\emph{Data availability:} all data essential for reproducing the
findings, that is, the machine-readable provenance graph
(\texttt{provenance.ttl}), the computed metrics
(\texttt{metrics.json}), the per-request telemetry export, the source
verification worksheet, and the full session transcripts, are openly
available in the version-pinned release v0.17.0 of the repository at
\url{https://github.com/noheton/f-ai2-r} and are archived on Zenodo
under a persistent identifier~\cite{fair2r-data2026}. The skill
bundle and all analysis code are available in the same release; the
skill loads in any Agent Skills client, and conformance-gated
releases carry bundle, PDF, graph, and checksums. A preprint of this
article is available as arXiv:2607.25637.
